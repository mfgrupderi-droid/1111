...
// siparisRoutes.js - Geliştirilmiş email endpoint'i
router.post('/:id/send-notification', async (req, res) => {
    try {
      const siparisId = req.params.id;
      
      // Sipariş ve şirket bilgilerini getir
      ...
      // Email servisini kullanarak gönderim yap
      const result = await sendSiparisNotification(siparis, sirket);
  
      // Siparişi güncelle (email gönderildi olarak işaretle)
