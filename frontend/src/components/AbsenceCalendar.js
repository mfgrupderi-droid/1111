...
const AbsenceCalendar = ({ employee, absences = [], onAddAbsence, onRemoveAbsence, onClose }) => {
    // Bugünün tarihini YYYY-MM-DD formatında alıyoruz (input için)
    const today = new Date().toISOString().split('T')[0];
    const [newDate, setNewDate] = useState(today);

    // Takvimde gösterilecek devamsız günleri Date objesine çeviriyoruz
    const absentDays = absences.map(absence => new Date(absence.date));

    const handleAddClick = () => {
        // Seçilen tarihin saat dilimi sorunları yaşamaması için UTC olarak ele alıyoruz
    ...
    return (
        <div className="modal-container">
            ...
            <div className="modal-body">
                ...
                {/* Sağ Taraf - Görsel Takvim */}
                <div className="calendar-panel">
                    <DayPicker
                        mode="multiple"
                        selected={absentDays}
                        locale={tr} // Takvimi Türkçeleştir
                        showOutsideDays
                        fixedWeeks
                        modifiersClassNames={{ selected: 'absent-day' }}
                        month={new Date()} // Her zaman güncel ayı göster
                    />
                </div>
            </div>
        </div>
