...
const EmployeeManager = () => {
    ...
    return (
        <div style={styles.container}>
            {loading && !showForm ? (
                <LoadingSpinner text="Çalışanlar yükleniyor..." />
            ) : (
                <>
                            ...
                            {employees.map((emp) => (
                                ...
                                >
                                    <div className="list-item-avatar">
                                        {emp.photo ? (
                                            ...
                                            <div className="avatar-placeholder">
                                                {generateAvatar(emp.firstName, emp.lastName)}
                                            </div>
                                        )}
                                    </div>

                                    <div className="list-item-content">
                                        <div className="list-item-name">{emp.firstName} {emp.lastName}</div>
                                        <div className="list-item-contact">{emp.phone}</div>
                                        {emp.email && <div className="list-item-email">{emp.email}</div>}
                                    </div>

                                    <div className="list-item-date">{formatDate(emp.createdAt)}</div>
                                    ...
                                    <div className="list-item-actions">
                                        ...
                                        <button 
                                            onClick={(e) => { e.stopPropagation(); handleDelete(emp._id || emp.id); }} 
                                            disabled={loading}
                            ...
                            <div className="detail-modal-body">
                                <div className="detail-modal-avatar">
                                    {selectedEmployee.photo ? (
                                        <img src={selectedEmployee.photo} alt={`${selectedEmployee.firstName} ${selectedEmployee.lastName}`} />
                                    ) : (
                                        <div className="detail-avatar-placeholder">
                                            {generateAvatar(selectedEmployee.firstName, selectedEmployee.lastName)}
                                        </div>
                                    )}
                                </div>
                                <h3 className="detail-modal-name">{selectedEmployee.firstName} {selectedEmployee.lastName}</h3>

                                <div className="detail-modal-info">
                                    <div className="detail-info-item">
                                        <Phone size={20} className="detail-info-icon" />
                                        <div>
                                            <p className="detail-info-label">Telefon</p>
                                            <p className="detail-info-value">{selectedEmployee.phone}</p>
                                        </div>
                                    </div>

                                    {selectedEmployee.email && (
                                        <div className="detail-info-item">
                                            <Mail size={20} className="detail-info-icon" />
                                            <div>
                                                <p className="detail-info-label">E-mail</p>
                                                <p className="detail-info-value">{selectedEmployee.email}</p>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <div className="detail-info-item">
                                        ...
                                        <div>
                                            <p className="detail-info-label">Eklenme Tarihi</p>
                                            <p className="detail-info-value">{formatDate(selectedEmployee.createdAt)}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="detail-modal-actions">
                                    <button 
                                        onClick={() => { setSelectedEmployee(null); handleEdit(selectedEmployee); }}
                                        disabled={loading}
