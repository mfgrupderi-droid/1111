...
const TEK_IS_FIYATI_DIKIM = 600.00;

const normalizeFiyat = (value) => {
    if (value === '' || value === null || value === undefined) return '';
    const num = parseFloat(value);
    if (Number.isNaN(num)) return value;
    if (num === 375) return 475;
    if (num === 410) return 510;
    return num;
};
...
function Hakedis() {
        ...
        const verileriYukle = async () => {
            ...
            try {
                ...
                if (fiyatResponse.ok) {
                    ...
                    const normalizeEdilmisFiyatlar = fiyatData.map(item => ({
                        ...item,
                        fiyat: normalizeFiyat(item.fiyat)
                    }));
                    setFiyatListesi(normalizeEdilmisFiyatlar);
                }
    ...
    const handleInputChange = (id, field, value) => {
        ...
        if (field === 'fiyat') {
            guncellenecekSatir.fiyat = normalizeFiyat(value);
        }
        ...
        if (field === 'adet' || field === 'model') {
            ...
            if (tekIsKuraliAktif) {
                ...
                guncellenecekSatir.fiyat = bulunanFiyat ? normalizeFiyat(bulunanFiyat.fiyat) : guncellenecekSatir.fiyat;
            }
        }

        const guncelAdet = parseInt(guncellenecekSatir.adet) || 0;
        const guncelFiyat = parseFloat(normalizeFiyat(guncellenecekSatir.fiyat)) || 0;
    ...
    const handleKaydet = async () => {
        ...
        try {
            const islemler = aktifVeri
                ...
                .map(satir => {
                    const normalizedFiyat = normalizeFiyat(satir.fiyat);
                    const fiyatValue = parseFloat(normalizedFiyat) || 0;
                    const adetValue = parseInt(satir.adet) || 0;
                    return {
                        model: satir.model,
                        adet: adetValue,
                        fiyat: fiyatValue,
                        toplam: adetValue * fiyatValue
                    };
                });
