...
function Sevkiyat() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [sirketRes, sevkiyatRes] = await Promise.all([
        ...
        axios.get(API_BASE_URL_SEVKIYAT, { params: { year } })
      ]);
  ...
  }, [API_BASE_URL_SEVKIYAT, API_BASE_URL_SIRKET, year]);
    ...
    <div style={{ padding: '24px' }}>
              ...
              <table className="sevkiyat-table">
                ...
                <tfoot>
                  <tr style={{backgroundColor: '#f0f4ff', fontWeight: '600', borderTop: '2px solid #e2e8f0'}}>
                    <td colSpan="3" className="action-column">TOPLAM</td>
                    <td>{filteredSevkiyatlar.reduce((sum, sevk) => sum + calculateTotalAdet(sevk.urunler), 0)}</td>
                    <td></td>
                    <td>{formatCurrency(filteredSevkiyatlar.reduce((sum, sevk) => sum + (sevk.toplamTutar || calculateTotalAmount(sevk.urunler)), 0))}</td>
      ...
      {isModalOpen && selectedSevkiyat && (
        <div className="modal-overlay" style={{ zIndex: 9999 }}>
          <div className="modal-content modal-large" style={{ 
            width: '95vw', 
            maxWidth: '1800px', 
            height: '90vh',
            display: 'flex',
            ...
            {/* KOMPAKT BAŞLIK - Tek satırda her şey */}
            <div style={{ 
              padding: '12px 20px', 
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '24px', fontSize: '14px' }}>
                <strong style={{ fontSize: '16px' }}>#{selectedSevkiyat.sevkiyatNo}</strong>
                <span>🏢 {selectedSevkiyat.sirketAdi}</span>
                <span>📅 {new Date(selectedSevkiyat.sevkiyatTarihi).toLocaleDateString('tr-TR')}</span>
                <span style={{ fontWeight: '600', fontSize: '15px' }}>
                  💰 {formatCurrency(selectedSevkiyat.toplamTutar || calculateTotalAmount(selectedSevkiyat.urunler))}
                </span>
              </div>
                ...
                <div key={filterType} style={{ position: 'relative' }}>
                  <button
                    ...
                    style={{
                      ...
                      border: '1px solid #e2e8f0',
                      borderRadius: '6px',
                      background: modalFilters[filterType].length > 0 ? '#3b82f6' : 'white',
                      color: modalFilters[filterType].length > 0 ? 'white' : '#475569',
                      fontSize: '13px',
                      cursor: 'pointer',
                      display: 'flex',
                      alignItems: 'center',
                  ...
                  >
                    ...
                    {modalFilters[filterType].length > 0 && (
                      <span style={{ 
                        ...
                        {modalFilters[filterType].length}
                      </span>
                    )}
                  </button>

                  <div 
                    id={`modal-${filterType}-dropdown`} 
                    style={{ 
                      display: 'none',
                      position: 'absolute',
                      top: '100%',
            ...
            <div style={{ 
              ...
              <table style={{ 
                ...
                <thead style={{ 
                  ...
                  <tr>
                    <th style={{ padding: '10px', textAlign: 'left', borderBottom: '2px solid #cbd5e1', fontWeight: '600' }}>Model</th>
                    <th style={{ padding: '10px', textAlign: 'left', borderBottom: '2px solid #cbd5e1', fontWeight: '600' }}>Cins</th>
                    <th style={{ padding: '10px', textAlign: 'left', borderBottom: '2px solid #cbd5e1', fontWeight: '600' }}>Renk</th>
                    {BEDENLER.map((beden, index) => (
                      <th key={index} style={{ 
                        padding: '10px 6px', 
                        textAlign: 'center', 
                    ...
                    <th style={{ padding: '10px', textAlign: 'center', borderBottom: '2px solid #cbd5e1', fontWeight: '600', background: '#dbeafe' }}>Toplam</th>
                    <th style={{ padding: '10px', textAlign: 'right', borderBottom: '2px solid #cbd5e1', fontWeight: '600' }}>Birim</th>
                    <th style={{ padding: '10px', textAlign: 'right', borderBottom: '2px solid #cbd5e1', fontWeight: '600', background: '#fef3c7' }}>Tutar</th>
                    <th style={{ padding: '10px', textAlign: 'left', borderBottom: '2px solid #cbd5e1', fontWeight: '600' }}>Not</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredModalUrunler.length > 0 ? (
                    filteredModalUrunler.map((urun, index) => {
                      ...
                      return (
                        <tr key={index} style={{ background: index % 2 === 1 ? '#f8fafc' : 'white' }}>
                          <td style={{ padding: '10px', borderBottom: '1px solid #e2e8f0', fontWeight: '600', color: '#1e40af' }}>{urun.model}</td>
                          <td style={{ padding: '10px', borderBottom: '1px solid #e2e8f0' }}>{urun.cins}</td>
                          <td style={{ padding: '10px', borderBottom: '1px solid #e2e8f0' }}>{urun.renk}</td>
                          {BEDENLER.map((beden, bedenIndex) => {
                            const bedenAdet = (urun.bedenler || []).find(b => b.beden === beden)?.adet;
                            const adetValue = parseInt(bedenAdet) || 0;
                            return (
                              <td key={bedenIndex} style={{ 
                                padding: '10px 6px', 
                                textAlign: 'center', 
                                borderBottom: '1px solid #e2e8f0',
                                fontWeight: adetValue > 0 ? '600' : 'normal',
                                color: adetValue > 0 ? '#16a34a' : '#94a3b8'
                              }}>
                          ...
                          <td style={{ padding: '10px', textAlign: 'right', borderBottom: '1px solid #e2e8f0' }}>
                            {formatCurrency(urun.birimFiyat || 0)}
                          </td>
                          <td style={{ 
                            padding: '10px', 
                            textAlign: 'right', 
                          ...
                          <td style={{ padding: '10px', borderBottom: '1px solid #e2e8f0', fontSize: '12px', color: '#64748b' }}>
                            {urun.not || '-'}
                          </td>
                        </tr>
                      );
                    ...
                    <tr>
                      <td colSpan={BEDENLER.length + 8} style={{ 
                        padding: '40px', 
                        textAlign: 'center',
                        color: '#94a3b8',
                        fontSize: '14px'
                ...
                <tfoot style={{ position: 'sticky', bottom: 0, background: '#f1f5f9', zIndex: 10 }}>
                  <tr>
                    <td colSpan={BEDENLER.length + 5} style={{ 
                      padding: '14px 20px', 
                      textAlign: 'right',
                      fontWeight: '700',
                      fontSize: '15px',
                    ...
                    <td style={{ 
                      ...
                      {formatCurrency(filteredModalUrunler.reduce((sum, urun) => {
            ...
            <div style={{ 
              ...
              <button onClick={() => generatePDF(selectedSevkiyat)} style={{
                padding: '8px 16px',
                background: '#dc2626',
                color: 'white',
                border: 'none',
              ...
              <button onClick={() => exportSevkiyatExcel(selectedSevkiyat)} style={{
                padding: '8px 16px',
                background: '#16a34a',
                color: 'white',
                border: 'none',
              ...
              <button onClick={() => handleSendNotification(selectedSevkiyat._id)} style={{
                padding: '8px 16px',
                background: '#2563eb',
                color: 'white',
                border: 'none',
