...
const [balanceModalTab, setBalanceModalTab] = useState('borcu'); 
...
const handleShowBalanceModal = async (sirket) => {
    setSelectedBalanceData(sirket);
    setShowBalanceModal(true);
    setBalanceModalTab('borcu');
    
    
    await loadFullHistoryAndCalculateBalances(sirket);
};
...
const handleSelectSirket = async (sirket) => {
    ...
    try {
        ...
        setSelectedSirket(response.data.sirket);
        await loadFullHistoryAndCalculateBalances(response.data.sirket); 
    } catch (error) {
    ...
    const handleUpdateIslem = async (e) => {
        ...
        try {
            ...
            await axios.put(`${API_URL_SIRKETLER}/islem/${selectedSirket._id}/${editingIslem._id}`, {
                islemTuru: transactionForm.islemTuru, 
                tutar: newTutar,                      
                aciklama: transactionForm.aciklama,   
                odemeTuru: transactionForm.odemeTuru  
            });
...
const exportToPDF = async () => {
  ...
  try {
    ...
    const sayfaBasinaIslem = 22; // Daha güvenli sayfa başına satır sayısı
    ...
    const formatTutar = (tutar) => {
      ...
      const sign = tutar > 0 ? '+' : '';
      return `${sign}${tutar.toLocaleString('tr-TR', {
        ...
        ">
          ...
          <div style="border-bottom: 2px solid #2c3e50; padding-bottom: 12px; margin-bottom: 16px;">
            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
              <div>
                <h1 style="margin: 0 0 4px 0; font-size: 16px; font-weight: 600; color: #2c3e50;">
                  BOZKURTSAN DERİ SAN. TİC. LTD. ŞTİ.
                </h1>
                <p style="margin: 0; font-size: 9px; color: #5a6c7d;">
                  Yenişehir Mahallesi 1245 Sokak. No: 31 Konak, İzmir
                </p>
              </div>
              <div style="text-align: right;">
                <h2 style="margin: 0 0 4px 0; font-size: 13px; font-weight: 600; color: #34495e;">
                  HESAP EKSTRESİ
                </h2>
                <p style="margin: 0; font-size: 9px; color: #5a6c7d;">
            ...
            <div style="margin-top: 12px; background: #f8f9fa; padding: 10px; border-radius: 4px;">
              <div style="display: flex; justify-content: space-between;">
                <div>
                  <p style="margin: 0 0 3px 0; font-size: 10px; font-weight: 600; color: #2c3e50;">
                    ${selectedSirket.sirketAdi}
                  </p>
                  <p style="margin: 0; font-size: 9px; color: #5a6c7d;">
                    Cari Kodu: ${selectedSirket.sirketKodu}
                  </p>
                </div>
                <div style="text-align: right;">
                  <p style="margin: 0; font-size: 9px; color: #5a6c7d;">
            ...
            ">
              <thead>
                <tr style="background: #34495e; color: white;">
                  <th style="padding: 8px 6px; text-align: left; font-weight: 600; width: 12%;">Tarih</th>
                  <th style="padding: 8px 6px; text-align: left; font-weight: 600; width: 38%;">Açıklama</th>
                  <th style="padding: 8px 6px; text-align: right; font-weight: 600; width: 16%;">Tutar (${paraBirimi})</th>
                  <th style="padding: 8px 6px; text-align: right; font-weight: 600; width: 17%;">Önceki Bakiye</th>
                  <th style="padding: 8px 6px; text-align: right; font-weight: 600; width: 17%;">Sonraki Bakiye</th>
                </tr>
              </thead>
              <tbody>
                ${sayfaIslemleri.map((item, idx) => `
                  <tr style="border-bottom: 1px solid #e9ecef; ${idx % 2 === 0 ? 'background: #f8f9fa;' : ''}">
                    <td style="padding: 7px 6px; color: #2c3e50;">${formatDate(item.tarih)}</td>
                    <td style="padding: 7px 6px; color: #2c3e50; word-wrap: break-word;">${item.islemAciklamasi || item.aciklama || '-'}</td>
                    <td style="padding: 7px 6px; text-align: right; color: ${item.gercekTutar > 0 ? '#27ae60' : item.gercekTutar < 0 ? '#e74c3c' : '#2c3e50'}; font-weight: 500;">
                      ${formatTutar(item.gercekTutar)}
                    </td>
                    <td style="padding: 7px 6px; text-align: right; color: #5a6c7d;">
                      ${formatBakiye(item.previousBalance)}
                    </td>
                    <td style="padding: 7px 6px; text-align: right; color: #2c3e50; font-weight: 500;">
                      ${formatBakiye(item.currentBalance)}
                    </td>
                  </tr>
                `).join('')}
              </tbody>
          ...
          <div style="
            ...
            align-items: center;
          ">
            <p style="margin: 0; font-size: 8px; color: #95a5a6;">
              Bu hesap ekstresi elektronik ortamda oluşturulmuştur.
            </p>
            <p style="margin: 0; font-size: 8px; color: #5a6c7d; font-weight: 500;">
              Sayfa ${s + 1} / ${toplamSayfa}
            </p>
          </div>
        </div>
      `;
    ...
    await html2pdf()
      .set({
        margin: 0,
        filename: `${selectedSirket.sirketAdi.replace(/[^a-zA-Z0-9ğüşıöçĞÜŞİÖÇ\s]/g, '')}_Ekstre_${bugununTarihi}.pdf`,
        jsPDF: {
          unit: 'mm',
          format: 'a4',
    ...
    return (
        <div style={{ position: 'relative', padding: '24px' }}>
            ...
            <div 
                ...
                onClick={() => setCurrencyView(prev => prev === 'TRY' ? 'USD' : 'TRY')}
            ...
            >
                ...
                <div className="summary-content">
                    <div className="summary-label">
                        Toplam Borç ({currencyView})
                        <span className="toggle-hint">Değiştirmek için tıklayın</span>
                    </div>
                    <div className="summary-amount negative">
                        {currencyView === 'TRY' ? (
                            <>
                                {sirketler
                                    .filter(s => s.sirketCariBirimi === 'TRY' && (s.sirketCarisi || 0) < 0)
                                    .reduce((sum, s) => sum + Math.abs(s.sirketCarisi || 0), 0)
                                    .toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺
                            </>
                        ) : (
                            <>
                                {sirketler
                                    .filter(s => s.sirketCariBirimi === 'USD' && (s.sirketCarisi || 0) < 0)
                                    .reduce((sum, s) => sum + Math.abs(s.sirketCarisi || 0), 0)
                    ...
                    <div className="summary-subtitle">
                        {sirketler.filter(s => s.sirketCariBirimi === currencyView && (s.sirketCarisi || 0) < 0).length} şirket
                    </div>
                </div>
            </div>
                    ...
                    <div className="summary-amount positive">
                        {currencyView === 'TRY' ? (
                            <>
                                {sirketler
                                    .filter(s => s.sirketCariBirimi === 'TRY' && (s.sirketCarisi || 0) > 0)
                                    .reduce((sum, s) => sum + (s.sirketCarisi || 0), 0)
                                    .toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺
                            </>
                        ) : (
                            <>
            ...
            >
                ...
                <div className="summary-content">
                    <div className="summary-label">
                        Net Bakiye ({currencyView})
                        <span className="toggle-hint">Değiştirmek için tıklayın</span>
                    </div>
                    <div className={`summary-amount ${
                        sirketler
                            .filter(s => s.sirketCariBirimi === currencyView)
                            .reduce((sum, s) => sum + (s.sirketCarisi || 0), 0) >= 0 
                            ? 'positive' 
                            : 'negative'
                    }`}>
                        {currencyView === 'TRY' ? (
                            <>
                                {sirketler
                                    .filter(s => s.sirketCariBirimi === 'TRY')
                                    .reduce((sum, s) => sum + (s.sirketCarisi || 0), 0)
                                    .toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺
                            </>
                        ) : (
                            <>
                                {sirketler
                                    .filter(s => s.sirketCariBirimi === 'USD')
                                    .reduce((sum, s) => sum + (s.sirketCarisi || 0), 0)
                                    .toLocaleString('en-US', { minimumFractionDigits: 2 })} $
                            </>
                        )}
                    ...
                    <div className="modern-companies-grid">
                        {getSirketPagination().currentSirketler.map(sirket => (
                            <div
                                key={sirket._id}
                            ...
                            >
                                ...
                                <div className="company-card-body">
                                    ...
                                    <div 
                                        className="company-balance-section"
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            handleShowBalanceModal(sirket);
                                        }}
                                        style={{ cursor: 'pointer' }}
                                    >
                        ...
                        {(() => {
                            const { totalPages } = getSirketPagination();
                            return totalPages > 1 && (
                            <div className="modern-pagination">
                                ...
                                <button
                                    ...
                                    onClick={() => setSirketPage(prev => Math.min(prev + 1, getSirketPagination().totalPages))}
                                    disabled={sirketPage === getSirketPagination().totalPages}
                                >
                                    <span>Sonraki</span>
                        ...
                        {(() => {
                            const { totalSirketler } = getSirketPagination();
                            return totalSirketler === 0 && (
    ...
    <div className="modern-history-section">
        {isHistoryLoading ? (
            ...
            {}
            {showBalanceModal && selectedBalanceData && (
                <div className="modal-overlay">
                    <div className="modal balance-modal">
                        <div className="modal-header">
                            <h3>{selectedBalanceData.sirketAdi}</h3>
                            <button
                                type="button"
                                className="modal-close-btn"
                                onClick={() => {
                        ...
                        <div className="balance-modal-tabs">
                            <button 
                                className={`balance-tab ${balanceModalTab === 'borcu' ? 'active' : ''}`}
                                onClick={() => setBalanceModalTab('borcu')}
                            >
                                Kime Borçluyuz
                            </button>
                            <button 
                                className={`balance-tab ${balanceModalTab === 'alacagi' ? 'active' : ''}`}
                                onClick={() => setBalanceModalTab('alacagi')}
                            >
                                Kimden Alacaklıyız
                            </button>
                        </div>
                        <div className="modal-content-x">
                            {balanceModalTab === 'borcu' ? (
                                <div className="balance-content">
                                    <p style={{marginTop: 0, fontSize: '14px', color: '#666'}}>
                                        Toplam Borç: <strong style={{color: '#e74c3c', fontSize: '18px'}}>
                                            {Math.max(0, -(selectedBalanceData.sirketCarisi || 0)).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} {selectedBalanceData.sirketCariBirimi}
                                        </strong>
                                    </p>
                                    {allCompanyTransactions.length > 0 ? (
                                        <table className="balance-transactions-table">
                                            <thead>
                                                <tr>
                                                    <th>Tarih</th>
                                                    <th>İşlem Türü</th>
                                            ...
                                            <tbody>
                                                {allCompanyTransactions
                                                    .filter(t => (t.gercekTutar || 0) > 0)
                                                    .map((islem, idx) => (
                                                        <tr key={idx}>
                                                            <td style={{fontSize: '13px'}}>{new Date(islem.tarih).toLocaleDateString('tr-TR')}</td>
                                                            <td style={{fontSize: '13px'}}>{islem.tip}</td>
                                                            <td style={{textAlign: 'right', fontSize: '13px', fontWeight: 500}}>
                                                                {(islem.gercekTutar || 0).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} {islem.paraBirimi}
                                                            </td>
                                                        </tr>
                                                    ))}
                                            </tbody>
                                ...
                                <div className="balance-content">
                                    <p style={{marginTop: 0, fontSize: '14px', color: '#666'}}>
                                        Toplam Alacak: <strong style={{color: '#27ae60', fontSize: '18px'}}>
                                            {Math.max(0, selectedBalanceData.sirketCarisi || 0).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} {selectedBalanceData.sirketCariBirimi}
                                        </strong>
                                    </p>
                                    {allCompanyTransactions.length > 0 ? (
                                                ...
                                                {allCompanyTransactions
                                                    ...
                                                    .map((islem, idx) => (
                                                        <tr key={idx}>
                                                            ...
                                                            <td style={{fontSize: '13px'}}>{islem.tip}</td>
                                                            <td style={{textAlign: 'right', fontSize: '13px', fontWeight: 500}}>
                                                                {Math.abs(islem.gercekTutar || 0).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} {islem.paraBirimi}
                                                            </td>
                                                        </tr>
                                                    ))}
                                            </tbody>
...
.balance-transactions-table thead th {
    background-color: #f8f9fa;
    color: #495057;
    padding: 10px;
    text-align: left;
...
.balance-transactions-table tbody td {
    padding: 10px;
    border-bottom: 1px solid #f1f1f1;
}

.balance-transactions-table tbody tr:hover {
    background-color: #f8f9fa;
}

.balance-transactions-table tbody tr:last-child td {
    border-bottom: none;
}
...
.summary-card.debt .summary-icon {
    background: linear-gradient(135deg, #ff6b6b, #ee5a52);
    color: white;
}
